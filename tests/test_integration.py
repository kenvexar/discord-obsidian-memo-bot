...
class MockMessage:
    ...
    def __init__(self, content: str, author_id: str = "user123", channel_id: int = 12345):
        self.id = 123456789
...
class TestCompleteMessageProcessingFlow:
    ...
    async def test_end_to_end_message_processing(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('src.ai.gemini_client.GeminiClient') as mock_gemini:
                ...
                # Obsidianファイル作成をモック
                with patch('src.obsidian.file_manager.ObsidianFileManager') as mock_obsidian:
    ...
    async def test_api_limit_handling(self):
        ...
        with patch('src.ai.gemini_client.GeminiClient') as mock_gemini:
            mock_gemini_instance = AsyncMock()
            mock_gemini_instance.process_message.side_effect = Exception("API quota exceeded")
            mock_gemini.return_value = mock_gemini_instance
